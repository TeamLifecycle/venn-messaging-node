...
function OneSignal(keys){
  ...
  this.send = function(data, callback) {
    var context = this;
    var ios = 0;
    request( getRegisterOptions(context.keys.app_id, ios, data.deviceToken), function(err, result) {
      request( getSendOptions(context.keys.app_id, context.keys.api_key, data.deviceToken, data.message), function(err, result) {
        if (err) {
...
getRegisterOptions = function(appId, deviceType, token) {
  var options = {
...
getSendOptions = function(appId, apiKey, token, message) {
  var options = {
