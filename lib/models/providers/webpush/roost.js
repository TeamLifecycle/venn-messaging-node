...
function Roost(keys) {
  ...
  this.send = function (data, callback) {
    ...
    // Validate data
    var validState = validateRoostData(data);
    if (!validState.valid) {
      return callback(validState.status);
    }
...
@return                 JSON object containing the details of validation
*/
validateRoostData = function (data) {
  var validState = {};

  if (!data.message || typeof data.message === 'undefined') {
    validState.valid = false;
    validState.status = new RoostUserStatus('`message` key required.', 'missing');

  } else if (!data.url || typeof data.url === 'undefined') {
    validState.valid = false;
    validState.status = new RoostUserStatus('`url` key required.', 'missing');

  } else {
    validState.valid = true;
    validState.status = new RoostUserStatus('Ok.', 'ok');
  }
