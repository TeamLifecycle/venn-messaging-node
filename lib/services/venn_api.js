...
//TODO: Change this file to include postal services
...
vennApiService.init = function (db) {
  ...
  if (context.db !== null && context.db !== undefined) {
    ...
    context.AppIntegration = db.model('AppIntegration');
...
getKeysFromDB = function (apikey, type, callback) {
  getUserApp(apikey, function (err, userapp) {
    ...
    context.IntegrationType.findOne({slug: type}, function (err, integrationType) {
      ...
      context.AppIntegration.find({app: userapp._id, "integration.type._id": integrationType._id, "active": true}, function (err, appIntegrations) {
        if (err) callback(err, null);
...
postLogFromDB = function (apikey, log, callback) {
  getUserApp(apikey, function (err, userapp) {
    ...
    log.forEach(function (entry) {
      context.AppIntegration.findOne({'app': userapp._id, 'integration.slug': entry.service}, function (err, appIntegration) {
        if (appIntegration) {
          context.AppLog.create({'app': userapp._id, 'appIntegration': appIntegration._id, 'code': entry.code, 'message': entry.message}, function (err, appLog) {
