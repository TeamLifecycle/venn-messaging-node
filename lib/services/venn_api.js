...
vennApiService.init = function (db) {
  ...
  if (db) {
    App = db.model('App');
    IntegrationType = db.model('IntegrationType');
    AppIntegration = db.model('AppIntegration');
    AppLog = db.model('AppLog');
  }
}
...
getUserApp = function (apikey, callback) {
  App.findOne({"key": apikey}, function (err, app) {
    callback(err, app);
  })
}
...
getKeysFromDB = function (apikey, type, callback) {
  getUserApp(apikey, function (err, userapp) {
    ...
    IntegrationType.findOne({slug: type}, function (err, integrationType) {
      if (err) callback(err, null);

      AppIntegration.find({app: req.userapp._id, "integration.type._id": integrationType._id, "active": true}, function (err, appIntegrations) {
        if (err) callback(err, null);
...
postLogFromDB = function (apikey, log, callback) {
  getUserApp(apikey, function (err, userapp) {
    ...
    log.forEach(function (entry) {
      AppIntegration.findOne({'app': userapp._id, 'integration.slug': entry.service}, function (err, appIntegration) {
        if (appIntegration) {
          AppLog.create({'app': req.userapp._id, 'appIntegration': appIntegration._id, 'code': entry.code, 'message': entry.message}, function (err, appLog) {
